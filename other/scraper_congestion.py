...
def process_video_stream_image(args, mac_id, video_flv_url, status):
    os.makedirs(str(mac_id), exist_ok=True)
        ...
        try:   
            ...
            while cap.isOpened():
                ...
                if frame_count % process_every_n_frames == 0:
                    image_path = os.path.join(str(mac_id), str(time.time()).replace('.', str(frame_count))+'.jpg')
    ...
    try:
        ...
        while cap.isOpened():
            ...
            if cnt % 360 == 0:
                ...
                # 创建新的视频文件名
                video_file_name = f'video/{str(mac_id)}_2_{video_count}.mp4'
...
if __name__ == '__main__':
    args = {
        ...
        'mac_id': [
            37021400001310142374,       # 银河路-球机
            37021400001310812117,       # 城阳新建12
            37021400001310511396,       # 世贸别墅南
            37021400001310253424,       # 仙山西路三岔口
            37021400001310500412,       # 正阳西路龙海路
            37021400001310967824,       # 大周村 
            37021400001310481380,       # 东田社区-全景
            37021400001310632299,       # 银河路-全景
            37021400001310068382        # 王沙路-全景
        ]
    }
    ...
    for mac_id in args['mac_id']:
        status.add_failed(mac_id)
        url = "https://iot.beiangeovis.top:443/rtp/37020000002006139128_{}.live.mp4".format(str(mac_id))
        # url = 'https://iot.beiangeovis.top:443/rtp/37020000002006139128_37021400001310142374.live.flv'
        if args['image']:
            thread = threading.Thread(target=process_video_stream_image, args=(args, mac_id, url, status))
        if args['video']:
            thread = threading.Thread(target=process_video_stream_video, args=(args, mac_id, url, status))   
